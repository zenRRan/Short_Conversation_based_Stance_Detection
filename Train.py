...
class Labeler:
    ...
    def train(self, train_file, dev_file=None, test_file=None):
        ...
        if self.HyperParams.Adam:
            print("using Adam...")
            Optimizer = oprim.Adam(model.parameters(), lr=lr, weight_decay=self.HyperParams.decay)
        if self.HyperParams.SGD:
            print("using SGD...")
            Optimizer = oprim.SGD(model.parameters(), lr=lr, weight_decay=self.HyperParams.decay)
        if Optimizer == None:
            print("please select a model!")
            return
